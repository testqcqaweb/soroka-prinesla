/**
 * Cloudflare Worker: принимает заявки с сайта и шлёт в Telegram (Архивариус).
...
export async function handleLeadRequest(request, env, fetchImpl = fetch) {
  ...
  try {
    ...
    if (!token || !chatId) {
      return json(
        {
          error: "Бот не настроен. Нужны TELEGRAM_BOT_TOKEN и TELEGRAM_CHAT_ID.",
        },
        503,
      );
    }

    const tg = await fetchImpl(`https://api.telegram.org/bot${token}/sendMessage`, {
      method: "POST",
    ...
    if (!tg.ok || !tgBody.ok) {
      return json(
        {
          error: "Telegram не принял сообщение. Проверьте токен бота и chat_id.",
